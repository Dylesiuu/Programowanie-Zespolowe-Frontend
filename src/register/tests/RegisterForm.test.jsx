...
describe("RegisterForm", () => {
  ...
  it("shows an error message when passwords do not match", async () => {
    ...
    await userEvent.type(screen.getByPlaceholderText("Powtórz hasło"), "DifferentPassword123!");
    await userEvent.click(screen.getByRole("button", { name: "Zarejestruj" }));
  ...
  it("shows an error message when email is already taken", async () => {
    ...
    await userEvent.type(screen.getByPlaceholderText("Powtórz hasło"), "StrongPassword123!");
    await userEvent.click(screen.getByRole("button", { name: "Zarejestruj" }));

    await waitFor(() => {
      expect(screen.getByText("Dany email jest już zajęty.")).toBeInTheDocument();
  ...
  it("shows an error message when registration fails", async () => {
    global.fetch = jest.fn(() =>
        Promise.resolve({
          ok: false,
          json: () => Promise.resolve({ message: "Wystąpił problem podczas rejestracji" }),
        })
    );
    ...
    await waitFor(() => {
      expect(screen.getByText("Wystąpił problem podczas rejestracji.")).toBeInTheDocument();
    });
  });
});